...
// Function to scrape content to URL
export async function scrapeUrl(url:string) {
    try {
    logger.info(`Staring scrape process for : ${url}`);
...
async function getCachedContent(url: string): Promise<ScrapedContent | null > {
    ...
    logger.info(`Cache hit - Found Cache content for: ${url}`);

    // Handle both string and object responses from Redis
    // estint-disable-next-line @typescript-estint/no-explicit-any
    let
    parsed: any;
