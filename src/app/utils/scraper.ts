...
export async function scrapeUrl(url: string) {
    try {
        ...
        const mainText = $("main").map((_, el) => $(el).text()).get().join(" ");
        ...
        const finalResponse = {
            ...
            error: null,
            cachedAt: Date.now(), // Add the timestamp for caching
        };

        await cacheContent(url, finalResponse);

        return finalResponse; // Return the final response after caching
...
async function getCachedContent(url: string): Promise<ScrapedContent | null> {
    try {
        ...
        const cached = await redis.get(cacheKey) as string | null; // Type assertion
...
async function cacheContent(url: string, content: ScrapedContent): Promise<void> {
    try {
        ...
        logger.info(`Successfully cached content for: ${url} (${serialized.length} bytes, TTL: ${CACHE_TTL})`);
